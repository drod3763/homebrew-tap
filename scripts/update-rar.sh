...
FORMULA_PATH="$formula_path" VERSION="$version" COMPACT_VERSION="$compact_version" ARM_SHA="$arm_sha" INTEL_SHA="$intel_sha" ruby <<'RUBY'
...
updated.sub!(/  if Hardware::CPU\.arm\?\n.*?\n  end\n\n/m, arch_block)
